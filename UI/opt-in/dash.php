...
<div id="wi-registration-used" style="margin: 10px auto; width:50%;">
	...
	<div class="progress-information">
		<p class="text-colour--faded-60"><?php echo absint( $user_count ); ?></p>
		...
		<?php if ( $progress < 100 ) : ?>
			<?php if ( ( $limit_users < $total_number_of_days && $progress < 80 ) || ( $limit_users > $total_number_of_days && $current_count_ratio < $total_count_ratio ) ) : ?>
					<?php 
						...
						printf( esc_html__( '%s Registrations left until user can not register!', 'webinarignition' ), absint( $user_left ) ); 
						?>
					<br>
				...
				$read_more = sprintf( '<a class="wi-dashboard-link" href="https://webinarignition.tawk.help/article/webinar-is-full-please-contact-the-webinar-host" target="_blank">%s</a>', $link, __( 'Details', 'webinarignition' ) );
				?>
					<?php 
